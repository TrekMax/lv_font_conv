...
template <typename glyphVector>
static void WriteCharGlyphToCFile(FILE *file, glyphVector *glyph);

template <typename glyphVector>
static void WriteUnicodeListToCFile(FILE *file, glyphVector *glyph);

static void ConverCharToBinFileLevel0(const char16_t *unicode, int unicodeLen, FILE *cFile, FILE *binFile, TrueTypeFont *font);
static void ConverCharToBinFileLevel1(const char16_t *unicode, int unicodeLen, FILE *cFile, FILE *binFile, TrueTypeFont *font);
...
bool ConverCharToCFile(string &letters, FILE *file, TrueTypeFont *font, string fontName, uint32_t cfg)
{
    ...
    // 写入字符信息映射图cmap
    fprintf(file, templateCmapFmt.c_str(),
            letterUnicode[0],
            letterUnicode[letterNum - 1],
            letterNum);

    // 写入cache定义，lvgl-v8以上版本会使用到
    ...
    // 写入字体信息
    fprintf(file, templateLvFontFmt.c_str(), font->GetFontName().c_str(), font->GetFontSize(), fontName.c_str(), font->GetFontSize());
...
bool ConverCharToBinFile(string &letters, FILE *cFile, FILE *binFile, TrueTypeFont *font, string fontName, uint32_t cfg)
{
    ...
    // 写入字体信息
    fprintf(cFile, templateBinLvFontFmt.c_str(), font->GetFontName().c_str(), font->GetFontSize(), level, fontName.c_str(), font->GetFontSize());
...
/**
 * @brief 对输入的字符进行预处理，包括ascii、汉字添加、删除控制字符、排序和去重操作
 * @param letters 预处理的字符串
 * @param letterNum 保存处理过后的unicode字符数目
 * @retval 处理成功则返回unicode数组，失败返回nullptr
 * @note 返回的unicode数组再不使用后需使用 delete[] 释放
 */
static char16_t *ConverPreprocess(string &letters, int &letterNum, uint32_t cfg)
{
    // 加入ascii字符?
    ...
    // 除去控制字符
    auto prev = [](char16_t ch)
    { return ch < 0x20; }; // lambda表达式
...
static void WriteCharGlyphToCFile(FILE *file, glyphVector *glyph)
{
    ...
    while (it != glyph->end())
    {
        fprintf(file, templateGlyphDscFmt.c_str(),
                it->bitmap_index,
                it->adv_w,
                it->box_h,
                it->box_w,

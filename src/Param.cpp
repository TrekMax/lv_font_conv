...
ConverParam *ConverParamDecode(int argc, const char *argv[], int *error)
{
    ...
    param.zHanAll = false;
    param.zHanCom = false;
    param.outPath = "./export/";
    param.outFileName = "lv_user_font";
    ...
    // 创建不存在的路径
    string command = "mkdir -p " + param.outPath;
...
const char *helpInfo = "\
\
lv_font_conv: version 0.2\n\
\t-o\t\t指定输出文件名, 同时作为变量名(文件名不包括后缀)\n\
\t\t\t\t若不指定该参数则默认为 lv_user_font\n\
\t\t\t\t此选项还可以指定输出路径,例:\"./export/lv_user_font(文件名不要加后缀)\"\n\
\t-i\t\t指定输入文件, 存有转换字符的 txt 文本文档(可选)\n\
\t-t\t\t指定 TrueType 字库文件(.ttf)，此为必要参数\n\
\t-c\t\t字体位图输出到 C 文件(数组), 与\"-b\"互斥(默认参数)\n\
\t-b\t\t字体位图输出为 Bin 文件, 与\"-c\"互斥\n\
\t--size\t\t指定字体尺寸(高度), 单位:像素, 默认值 24\n\
\t--bpp\t\t指定像素位数(抗锯齿), 可选的有 1,2,4(默认值),8\n\
\t--ascii\t\t转换前加入 ASCII 字符\n\
\t--zHanAll\t转换前加入所有汉字\n\
\t--zHanCom\t转换前加入常用汉字(约6000个)\n\
\t--level\t\t指定 -b 输出到bin文件时, 保存在C文件的字体信息等级, 可选的有 0 (默认值),1\n\
\t\t\t\t数值越大, C 文件保存的字体信息越多, 文件 IO 次数越少,\n\
\t\t\t\t渲染速度越快, 但 C 文件占用空间越大\n\
\n\
Tips: -i --ascii --zHanAll --zHanComm 四个选项中请至少指定一个选项,\n\
以确保有字符可转换.\n\
\
";
